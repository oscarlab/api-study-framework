...
BEGIN
	...
	RIGHT JOIN
	libc_symbol AS t2
	ON t1.bin_id = t2.bin_id AND t1.call = t2.symbol_name
...
\copy (SELECT symbol, get_pop(popularity) AS by_inst FROM query_output()) TO '/tmp/libc-popularity-by-inst.csv' WITH CSV HEADER
\echo 'Output to /tmp/libc-popularity-by-inst.csv'
